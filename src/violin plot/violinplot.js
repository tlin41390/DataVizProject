function main() {
    ...
    d3.csv("../../CSV/stock_and_time-(Nvidia).csv", function (data) {
        ...
        var y = d3.scaleLinear()
            .domain([0, 60]) // Note that here the Y scale is set manually
            .range([height, 0])
        ...
        var sumstat = d3.nest()  // nest function allows to group the calculation per level of a factor
            .key(function (d) { return d.chipset; })
            .rollup(function (d) {   // For each key..
                let input = d.map(function (g) { return g['time(minutes)']; })    // Keep the variable called Sepal_Length   // And compute the binning on it.
